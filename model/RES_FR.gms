...
and Battery, PHS (pumped hydro storage) and methanation (P2G) as storage technologies,including secondary reserve requirements;
...
parameter eta_out(str) 'discharging efficiency of storage technolgoies' /PHS 0.9, battery 0.95, methanation 0.63/;
...
lcos('methanation') = (CAPA.l('methanation')*fixed_costs('methanation')+S.l('methanation')*(s_capex('methanation')+s_opex('methanation'))+gene_tec('methanation')*vOM('methanation')+CAPACITY.l('methanation')*capex_en('methanation'))/gene_tec('methanation') + lcoe('gas');
...
parameter lake_dual(m) 'dual of lake inflows equation';
lake_dual(m) = 1000000*lake_res.m(m);
parameter bio_dual 'dual of biogas constraint';
bio_dual = 1000000*biogas_const.m;
parameter rsv_dual(h) 'dual of reserves'    ;
rsv_dual(h)= 1000000*reserves.m(h);
parameter mean_rsv 'average dual value of reserves';
mean_rsv = sum(h,rsv_dual(h))/8760;
*-------------------------------------------------------------------------------
display cost.l;
display capa.l;
display sumdemand;
...
$Offtext
*-------------------------------------------------------------------------------
file results /'outputs/summary.txt'/ ;
*the .txt file
put results;
...
//
'Duals'//
'for lake we have ' lake_dual('jan')','lake_dual('feb')','lake_dual('mar')','lake_dual('apr')','lake_dual('may')','lake_dual('jun') //
lake_dual('jul')','lake_dual('aug')','lake_dual('sep')','lake_dual('oct')','lake_dual('nov')','lake_dual('dec') //
'for biogas' bio_dual //
'for reserves' mean_rsv//
//
;
*-------------------------------------------------------------------------------
file hourly_generation /'outputs/profiles.csv' / ;
*the .csv file
parameter nSTORAGE(str,h);
nSTORAGE(str,h) = 0 - STORAGE.l(str,h);
put hourly_generation;
hourly_generation.pc=5;
put 'hour'; loop(tec, put tec.tl;); put 'demand' , 'Pump' , 'ElecStr' , 'CH4' , 'price', 'rsv_dual'/ ;
loop (h,
put h.tl; loop(tec,  put gene.l(tec,h);) put demand(h), nSTORAGE('PHS',h), nSTORAGE('battery',h), nSTORAGE('methanation',h), spot_price(h), rsv_dual(h)/
;);
*-------------------------------------------------------------------------------
*                                The End :D
*-------------------------------------------------------------------------------
